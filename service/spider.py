...
def eventSpider(layout):
    ...
    for item in items:
      ...
      if title is None:
        print(time['datetime'], '\n', brief.text, '\n\n')
        list.append(json.dumps({"date": time['datetime'], "title": "", "brief": strTrans(brief.text)}, ensure_ascii=False))
      else:
        print(time['datetime'], '\n', title.text, '\n', brief.text, '\n\n')
        list.append(json.dumps({"date": time['datetime'], "title": strTrans(title.text), "brief": strTrans(brief.text)}, ensure_ascii=False))
    return list
